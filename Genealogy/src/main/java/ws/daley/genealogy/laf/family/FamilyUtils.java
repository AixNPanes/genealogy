...
class FamilyUtils
{
	...
	/**
	 * This draws the "Flush 3D Border" which is used throughout the Family L&F
	 */
	...
	//
	/**
	 * Draws a radial level gradient. The gradient will be drawn vertically if
	...
	private static class GradientPainter extends CachedPainter
	{
		...
		public void paint(Component c, Graphics2D g, @SuppressWarnings("rawtypes") List gradient, int x, int y,
		        @SuppressWarnings("hiding") int w, @SuppressWarnings("hiding") int h, boolean isVertical)
		{
			int imageWidth;
			int imageHeight;
			if (isVertical)
		...
		@Override
		protected void paintToImage(@SuppressWarnings("unused") Component c, @SuppressWarnings("unused") Image image,
		        Graphics g, @SuppressWarnings("hiding") int w, @SuppressWarnings("hiding") int h, Object[] args)
		{
			...
			if (isVertical)
				drawVerticalGradient(g2, ((Number) gradient.get(0)).floatValue(),
				        ((Number) gradient.get(1)).floatValue(), (Color) gradient.get(2), (Color) gradient.get(3),
		...
		@Override
		protected void paintImage(@SuppressWarnings("unused") Component c, Graphics g, int x, int y,
		        @SuppressWarnings("unused") int imageW, @SuppressWarnings("unused") int imageH, Image image,
		        Object[] args)
		{
			boolean isVertical = ((Boolean) args[1]).booleanValue();
		...
		private void drawVerticalGradient(Graphics2D g, float ratio1, float ratio2, Color c1, Color c2, Color c3,
		        @SuppressWarnings("hiding") int w, @SuppressWarnings("hiding") int h)
		{
			int mid = (int) (ratio1 * h);
			int mid2 = (int) (ratio2 * h);
			if (mid > 0)
			{
				g.setPaint(getGradient(0, 0, c1, 0, mid, c2));
			...
			if (mid > 0)
			{
				g.setPaint(getGradient(0, (float) mid + mid2, c2, 0, (float) mid * 2 + mid2, c1));
				g.fillRect(0, mid + mid2, w, mid);
			}
			if (h - mid * 2 - mid2 > 0)
			{
				g.setPaint(getGradient(0, (float) mid * 2 + mid2, c1, 0, h, c3));
				g.fillRect(0, mid * 2 + mid2, w, h - mid * 2 - mid2);
			}
		}

		private void drawHorizontalGradient(Graphics2D g, float ratio1, float ratio2, Color c1, Color c2, Color c3,
		        @SuppressWarnings("hiding") int w, @SuppressWarnings("hiding") int h)
		{
			...
			if (mid > 0)
			{
				g.setPaint(getGradient(0, 0, c1, mid, 0, c2));
			...
			if (mid > 0)
			{
				g.setPaint(getGradient((float) mid + mid2, 0, c2, (float) mid * 2 + mid2, 0, c1));
				g.fillRect(mid + mid2, 0, mid, h);
			}
			if (w - mid * 2 - mid2 > 0)
			{
				g.setPaint(getGradient((float) mid * 2 + mid2, 0, c1, w, 0, c3));
	...
	private static class OceanDisabledButtonImageFilter extends RGBImageFilter
	{
		...
		@Override
		public int filterRGB(@SuppressWarnings("unused") int x, @SuppressWarnings("unused") int y, int rgb)
		{
			// Coefficients are from the sRGB color space:
			int gray = Math.min(255, (int) (((0.2125f * ((rgb >> 16) & 0xFF)) + (0.7154f * ((rgb >> 8) & 0xFF))
