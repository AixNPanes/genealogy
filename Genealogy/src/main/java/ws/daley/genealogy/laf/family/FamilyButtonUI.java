 ...
 * @author Tom Santos
 */
public class FamilyButtonUI extends BasicButtonUI
{
	...
	private static final Object FAMILY_BUTTON_UI_KEY = new Object();
	...
	public static ComponentUI createUI(@SuppressWarnings("unused") JComponent c)
	{
		...
		FamilyButtonUI familyButtonUI = (FamilyButtonUI) appContext.get(FAMILY_BUTTON_UI_KEY);
		if (familyButtonUI == null)
		{
			familyButtonUI = new FamilyButtonUI();
			appContext.put(FAMILY_BUTTON_UI_KEY, familyButtonUI);
		}
		return familyButtonUI;
	}
	...
	public void update(Graphics g, JComponent c)
	{
		...
		if ((c.getBackground() instanceof UIResource) && button.isContentAreaFilled() && c.isEnabled())
		{
			...
			{
				if (!model.isArmed() && !model.isPressed() && FamilyUtils.drawGradient(c, g, "Button.gradient", 0, 0, c.getWidth(), c.getHeight(), true))
				{
					paint(g, c);
					return;
				}
			}
			else if (model.isRollover() && FamilyUtils.drawGradient(c, g, "Button.gradient", 0, 0, c.getWidth(), c.getHeight(), true))
			{
				paint(g, c);
				return;
			}
