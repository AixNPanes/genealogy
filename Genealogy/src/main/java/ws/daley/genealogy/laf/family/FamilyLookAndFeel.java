...
/**
 * The Java Look and Feel, otherwise known as Family.
 * <p>
 * Each of the {@code ComponentUI}s provided by {@code
 * FamilyLookAndFeel} derives its behavior from the defaults table. Unless
 * otherwise noted each of the {@code ComponentUI} implementations in this
 * package myDocument the set of defaults they use. Unless otherwise noted the
 ...
 * <p>
 * {@code FamilyLookAndFeel} derives it's color palette and fonts from
 * {@code FamilyTheme}. The default theme is {@code FamilyOceanTheme}. The theme
 * can be changed using the {@code setCurrentTheme} method, refer to it for
 * details on changing the theme. Prior to 1.5 the default theme was
 * {@code DefaultFamilyTheme}. The system property {@code "swing.familyTheme"}
 * can be set to {@code "steel"} to indicate the default should be
 * {@code DefaultFamilyTheme}.
 * <p>
 ...
 *
 * @see FamilyTheme
 * @see DefaultFamilyTheme
 * @see FamilyOceanTheme
 *
 * @author Steve Wilson
 */
@SuppressWarnings("serial")
public class FamilyLookAndFeel extends BasicLookAndFeel
{
	private static boolean FAMILY_LOOK_AND_FEEL_INITED = false;
	private static final String FAMILY_LAF = "ws.daley.genealogy.laf.family.";

	private static boolean checkedWindows;	//True if checked for windows yet.
	private static boolean isWindows;		//True if running on Windows.

	@SuppressWarnings("unused")
	private static boolean checkedSystemFontSettings;	//Set to true first time we've checked swing.useSystemFontSettings.
	...
	static boolean isWindows()				//Returns true if running on Windows.
	{
		if (!checkedWindows)
		{
	...
	static boolean useSystemFonts()
	{
		...
		{
			if (FAMILY_LOOK_AND_FEEL_INITED)
			{
				Object value = UIManager.get("Application.useSystemFontSettings");
				return (value == null || Boolean.TRUE.equals(value));
			}
			// If an instanceof FamilyLookAndFeel hasn't been inited yet, we
	...
	{
		return (getCurrentTheme() instanceof FamilyOceanTheme);
	}

	/**
	 * Returns the name of this look and feel. This returns {@code "Family"}.
	 *
	...
	/**
	 * Returns {@code false}; {@code FamilyLookAndFeel} is not a native look and
	 * feel.
	 *
	 * @return {@code false}
	 */
	...
	/**
	 * Returns {@code true}; {@code FamilyLookAndFeel} can be run on any
	 * platform.
	 *
	 * @return {@code true}
	 */
	...
	/**
	 * Returns {@code true}; family can provide {@code Window} decorations.
	 *
	 * @return {@code true}
	 *
	 ...
	 * FamilyLookAndFeel} registers an entry for each of the classes in the
	 * package {@code ws.daley.genealogy.laf.family} that are named FamilyXXXUI.
	 * The string {@code XXX} is one of Swing's uiClassIDs. For the
	 * {@code uiClassIDs} that do not have a class in family, the corresponding
	 * class in {@code javax.swing.plaf.basic} is used. For example, family does
	 * not have a class named {@code
	 * "FamilyColorChooserUI"}, as such, {@code
	...
	protected void initClassDefaults(UIDefaults table)
	{
		...
		final String familyPackageName = FAMILY_LAF + "";

		Object[] uiDefaults = {
			"ButtonUI", familyPackageName + "FamilyButtonUI",
			"CheckBoxUI", familyPackageName + "FamilyCheckBoxUI",
			"ComboBoxUI", familyPackageName + "FamilyComboBoxUI",
			"DesktopIconUI", familyPackageName + "FamilyDesktopIconUI",
			"FileChooserUI", familyPackageName + "FamilyFileChooserUI",
			"InternalFrameUI", familyPackageName + "FamilyInternalFrameUI",
			"LabelUI", familyPackageName + "FamilyLabelUI",
			"PopupMenuSeparatorUI", familyPackageName + "FamilyPopupMenuSeparatorUI",
			"ProgressBarUI", familyPackageName + "FamilyProgressBarUI",
			"RadioButtonUI", familyPackageName + "FamilyRadioButtonUI",
			"ScrollBarUI", familyPackageName + "FamilyScrollBarUI",
			"ScrollPaneUI", familyPackageName + "FamilyScrollPaneUI",
			"SeparatorUI", familyPackageName + "FamilySeparatorUI",
			"SliderUI", familyPackageName + "FamilySliderUI",
			"SplitPaneUI", familyPackageName + "FamilySplitPaneUI",
			"TabbedPaneUI", familyPackageName + "FamilyTabbedPaneUI",
			"TextFieldUI",familyPackageName + "FamilyTextFieldUI",
			"ToggleButtonUI", familyPackageName + "FamilyToggleButtonUI",
			"ToolBarUI", familyPackageName + "FamilyToolBarUI",
			"ToolTipUI", familyPackageName + "FamilyToolTipUI",
			"TreeUI", familyPackageName + "FamilyTreeUI",
			"RootPaneUI", familyPackageName + "FamilyRootPaneUI"
		};
	 ...
	 * </table>
	 * The value {@code theme} corresponds to the current {@code FamilyTheme}.
	 *
	 * @param table
	...
	{
		FamilyTheme theme = getCurrentTheme();
	...
	{
		table.addResourceBundle(FAMILY_LAF + "family");
	}

	/**
	 * Populates {@code table} with the defaults for family.
	 *
	 * @param table
	...
	protected void initComponentDefaults(UIDefaults table)
	{
		...
		Object textFieldBorder = new SwingLazyValue( FAMILY_LAF + "FamilyBorders", "getTextFieldBorder");

		LazyValue dialogBorder = t -> new FamilyBorders.DialogBorder();

		LazyValue questionDialogBorder = t -> new FamilyBorders.QuestionDialogBorder();
		...
		Object scrollPaneBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders$ScrollPaneBorder");
		Object buttonBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders", "getButtonBorder");

		Object toggleButtonBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders", "getToggleButtonBorder");
		...
		Object desktopIconBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders", "getDesktopIconBorder");

		Object menuBarBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders$MenuBarBorder");

		Object popupMenuBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders$PopupMenuBorder");
		Object menuItemBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders$MenuItemBorder");

		Object menuItemAcceleratorDelimiter = "-";
		Object toolBarBorder = new SwingLazyValue(FAMILY_LAF + "FamilyBorders$ToolBarBorder");
		...
		Object[] defaultCueList = new Object[] { "OptionPane.errorSound", "OptionPane.informationSound", "OptionPane.questionSound",
		        "OptionPane.warningSound" };

		FamilyTheme theme = getCurrentTheme();
		Object menuTextValue = new FontActiveValue(theme, FamilyTheme.MENU_TEXT_FONT);
		Object controlTextValue = new FontActiveValue(theme, FamilyTheme.CONTROL_TEXT_FONT);
		Object userTextValue = new FontActiveValue(theme, FamilyTheme.USER_TEXT_FONT);
		Object windowTitleValue = new FontActiveValue(theme, FamilyTheme.WINDOW_TITLE_FONT);
		Object subTextValue = new FontActiveValue(theme, FamilyTheme.SUB_TEXT_FONT);
		Object systemTextValue = new FontActiveValue(theme, FamilyTheme.SYSTEM_TEXT_FONT);
		//
		// DEFAULTS TABLE
		//
		...
		Object[] defaults = {
		        ...
		        "CheckBox.icon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getCheckBoxIcon"),
		        ...
		        "RadioButton.icon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getRadioButtonIcon"),
		        ...
		        // File View
		        "FileView.directoryIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeFolderIcon"),
		        "FileView.fileIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeLeafIcon"),
		        "FileView.computerIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeComputerIcon"),
		        "FileView.hardDriveIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeHardDriveIcon"),
		        "FileView.floppyDriveIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeFloppyDriveIcon"),

		        // File Chooser
		        "FileChooser.detailsViewIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getFileChooserDetailViewIcon"),
		        "FileChooser.homeFolderIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getFileChooserHomeFolderIcon"),
		        "FileChooser.listViewIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getFileChooserListViewIcon"),
		        "FileChooser.newFolderIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getFileChooserNewFolderIcon"),
		        "FileChooser.upFolderIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getFileChooserUpFolderIcon"),
		        ...
		        "Slider.horizontalThumbIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getHorizontalSliderThumbIcon"),
		        "Slider.verticalThumbIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getVerticalSliderThumbIcon"),
				...
				"InternalFrame.icon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getInternalFrameDefaultMenuIcon"),
				"InternalFrame.border", new SwingLazyValue(FAMILY_LAF + "FamilyBorders$InternalFrameBorder"),
				"InternalFrame.optionDialogBorder", new SwingLazyValue(FAMILY_LAF + "FamilyBorders$OptionDialogBorder"),
				"InternalFrame.paletteBorder", new SwingLazyValue(FAMILY_LAF + "FamilyBorders$PaletteBorder"),
				"InternalFrame.paletteTitleHeight", new Integer(11),
				"InternalFrame.paletteCloseIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory$PaletteCloseIcon"),
				"InternalFrame.closeIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getInternalFrameCloseIcon", internalFrameIconArgs),
				"InternalFrame.maximizeIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getInternalFrameMaximizeIcon", internalFrameIconArgs),
				"InternalFrame.iconifyIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getInternalFrameMinimizeIcon", internalFrameIconArgs),
				"InternalFrame.minimizeIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getInternalFrameAltMaximizeIcon", internalFrameIconArgs),
				...
				"Table.ascendingSortIcon", SwingUtilities2.makeIcon(getClass(), FamilyLookAndFeel.class, "icons/sortUp.png"),
				"Table.descendingSortIcon", SwingUtilities2.makeIcon(getClass(), FamilyLookAndFeel.class, "icons/sortDown.png"),
				
				"TableHeader.font", userTextValue, "TableHeader.cellBorder", new SwingLazyValue( FAMILY_LAF + "FamilyBorders$TableHeaderBorder"),
				...
				"Menu.checkIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getMenuItemCheckIcon"),
				"Menu.arrowIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getMenuArrowIcon"),
				...
				"MenuItem.checkIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getMenuItemCheckIcon"),
				"MenuItem.arrowIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getMenuItemArrowIcon"),
				...
				// Option Pane Special Dialog Colors, used when FamilyRootPaneUI
				...
				"CheckBoxMenuItem.checkIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getCheckBoxMenuItemIcon"),
				"CheckBoxMenuItem.arrowIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getMenuItemArrowIcon"),
				...
				"RadioButtonMenuItem.checkIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getRadioButtonMenuItemIcon"),
				"RadioButtonMenuItem.arrowIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getMenuItemArrowIcon"),
				...
				"Tree.openIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeFolderIcon"),
				"Tree.closedIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeFolderIcon"),
				"Tree.leafIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeLeafIcon"),
				"Tree.expandedIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeControlIcon", new Object[] {Boolean.valueOf(FamilyIconFactory.DARK) }),
				"Tree.collapsedIcon", new SwingLazyValue(FAMILY_LAF + "FamilyIconFactory", "getTreeControlIcon", new Object[] {Boolean.valueOf(FamilyIconFactory.LIGHT) }),
				...
				"ToolBar.rolloverBorder", (LazyValue) t -> FamilyBorders.getToolBarRolloverBorder(),
				"ToolBar.nonrolloverBorder", (LazyValue) t -> FamilyBorders.getToolBarNonrolloverBorder(),
				...
				// RootPane
				"RootPane.frameBorder", (LazyValue) t -> new FamilyBorders.FrameBorder(),
				"RootPane.plainDialogBorder", dialogBorder,
				"RootPane.informationDialogBorder", dialogBorder,
				"RootPane.errorDialogBorder", (LazyValue) t -> new FamilyBorders.ErrorDialogBorder(),
				...
				"RootPane.warningDialogBorder", (LazyValue) t -> new FamilyBorders.WarningDialogBorder(),
		...
		{
			Object messageFont = new FamilyFontDesktopProperty("win.messagebox.font.height", FamilyTheme.CONTROL_TEXT_FONT);
	...
	/**
	 * Ensures the current {@code FamilyTheme} is {@code non-null}. This is a
	 ...
	 * @see FamilyTheme#addCustomEntriesToTable(UIDefaults)
	 */
	@Override
	public UIDefaults getDefaults()
	{
		// PENDING: move this to initialize when API changes are allowed
		FAMILY_LOOK_AND_FEEL_INITED = true;

		createDefaultTheme();
		UIDefaults table = super.getDefaults();
		FamilyTheme currentTheme = getCurrentTheme();
	...
	/**
	 * Set the theme used by <code>FamilyLookAndFeel</code>.
	 * <p>
	 * After the theme is set, {@code FamilyLookAndFeel} needs to be
	 * re-installed and the uis need to be recreated. The following shows how to
	 * do this:
	 * 
	 * <pre>
	 * FamilyLookAndFeel.setCurrentTheme(theme);
	 *
	 ...
	 * @see #getCurrentTheme
	 */
	public static void setCurrentTheme(FamilyTheme theme)
	{
		// NOTE: because you need to recreate the look and feel after
		// this step, we don't bother blowing away any potential windows
		// values.
		if (theme == null) { throw new NullPointerException("Can't have null theme"); }
		AppContext.getAppContext().put("currentFamilyTheme", theme);
	}

	/**
	 * Return the theme currently being used by <code>FamilyLookAndFeel</code>.
	 ...
	 * @since 1.5
	 */
	public static FamilyTheme getCurrentTheme()
	{
		FamilyTheme currentTheme;
		AppContext context = AppContext.getAppContext();
		currentTheme = (FamilyTheme) context.get("currentFamilyTheme");
		if (currentTheme == null)
		{
			// This will happen in two cases:
			// . When FamilyLookAndFeel is first being initialized.
			...
			if (useHighContrastTheme())
				currentTheme = new FamilyHighContrastTheme();
			else
			{
				// Create the default theme. We prefer Ocean, but will
				// use DefaultFamilyTheme if told to.
				String theme = AccessController.doPrivileged(
				        new GetPropertyAction("swing.familyTheme"));
				if ("steel".equals(theme))
					currentTheme = new DefaultFamilyTheme();
				else
					currentTheme = new FamilyOceanTheme();
			}
	...
	{
		if ((icon instanceof ImageIcon) && FamilyLookAndFeel.usingOcean())
			return FamilyUtils.getOceanDisabledButtonIcon(((ImageIcon) icon).getImage());
	...
	{
		return FamilyLayoutStyle.INSTANCE;
	}

	/**
	 * FontActiveValue redirects to the appropriate family theme method.
	 */
	private static class FontActiveValue implements UIDefaults.ActiveValue
	{
		private int type;
		private FamilyTheme theme;

		FontActiveValue(FamilyTheme theme, int type)
		{
			this.theme = theme;
			this.type = type;
		}
		...
		@Override
		public Object createValue(@SuppressWarnings("unused") UIDefaults table)
		{
			Object value = null;
			switch(this.type)
			{
				case FamilyTheme.CONTROL_TEXT_FONT:
					value = this.theme.getControlTextFont();
					break;
				case FamilyTheme.SYSTEM_TEXT_FONT:
					value = this.theme.getSystemTextFont();
					break;
				case FamilyTheme.USER_TEXT_FONT:
					value = this.theme.getUserTextFont();
					break;
				case FamilyTheme.MENU_TEXT_FONT:
					value = this.theme.getMenuTextFont();
					break;
				case FamilyTheme.WINDOW_TITLE_FONT:
					value = this.theme.getWindowTitleFont();
					break;
				case FamilyTheme.SUB_TEXT_FONT:
	...
	private static class FamilyLayoutStyle extends DefaultLayoutStyle
	{
		private static FamilyLayoutStyle INSTANCE = new FamilyLayoutStyle();
	...
	public static Object getUIOfType(ComponentUI ui, @SuppressWarnings("rawtypes") Class klass)
	{
