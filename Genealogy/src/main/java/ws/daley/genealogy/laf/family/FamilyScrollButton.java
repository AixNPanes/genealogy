...
/**
 * JButton object for Family scrollbar arrows.
 * <p>
...
public class FamilyScrollButton extends BasicArrowButton
{
	...
	public FamilyScrollButton(int direction, int width, boolean freeStanding)
	{
		super(direction);
	...
	{
		boolean leftToRight = FamilyUtils.isLeftToRight(this);
		boolean isEnabled = getParent().isEnabled();

		Color arrowColor = isEnabled ? FamilyLookAndFeel.getControlInfo() : FamilyLookAndFeel.getControlDisabled();
		...
		if (getDirection() == NORTH)
		{
			...
			if (isEnabled)
			{
				...
				FamilyUtils.drawDisabledBorder(g, 0, 0, width, height + 1);
			if (!this.isFreeStanding)
			{
				height -= 1;
				g.translate(0, 1);
		...
		else if (getDirection() == SOUTH)
		{
			...
			if (isEnabled)
			{
				...
				FamilyUtils.drawDisabledBorder(g, 0, -1, width, height + 1);
		...
		else if (getDirection() == EAST)
		{
			...
			if (isEnabled)
			{
				...
				FamilyUtils.drawDisabledBorder(g, -1, 0, width + 1, height);
			if (!this.isFreeStanding)
			{
				height -= 2;
				width -= 1;
		...
		else if (getDirection() == WEST)
		{
			...
			if (isEnabled)
			{
				...
				FamilyUtils.drawDisabledBorder(g, 0, 0, width + 1, height);
